...
function upload (req, callback) {
  ...
  busboy.on("file", function(fieldname, file, filename, encoding, mimetype) {
    ...
    var randomName = id + "." + getExtension(filename);
    ...
    diskWriterStream.on("finish", function() {
     ...
     let rowsImage = JSON.parse(rawdata);
     var obj = {
      'url' :  link,
      'thumb':  link,
      "tag": require.main.filename,
      "name": require.main.filename,
      "id": id
     }  
     rowsImage.push(obj)
     let data = JSON.stringify(rowsImage)
     
     fs.writeFile('images.json', data, (err) => {  
         if (err) throw err;
